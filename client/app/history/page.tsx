                      ...
                      <div className="relative h-16 w-16">
                        <svg className="h-full w-full" viewBox="0 0 100 100">
                          ...
                          <circle
                            ...
                            strokeDashoffset={
                              251.2 - (251.2 * analysis.score) / 100
                            }
                            strokeLinecap="round"
                            fill="transparent"
                            r="40"
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <span
                            className={`text-sm font-bold ${getScoreColor(analysis.score)}`}
                          >
                            {analysis.score}%
                          </span>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {analysis.keywords_match?.map((keyword, i) => (
                          <Badge
                            key={i}
                            variant="secondary"
                            className="bg-green-50 text-green-700 border-green-100 text-[10px]"
                          >
                            {keyword}
                          </Badge>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {analysis.keywords_missing?.map((keyword, i) => (
                          <Badge
                            key={i}
                            variant="secondary"
                            className="bg-red-50 text-red-700 border-red-100 text-[10px]"
                          >
                            {keyword}
                          </Badge>
                        ))}
                  ...
                  <div className="pt-2 border-t">
                    ...
                    <div className="flex flex-col gap-1.5">
                      {analysis.suggestions?.map((s, i) => (
                        <div
                          key={i}
                          className="text-xs p-2.5 rounded-lg bg-blue-50/50 border border-blue-100/50 text-blue-800 flex gap-2"
                        >
